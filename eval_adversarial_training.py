...
parser.add_argument("--strength", type=int, default=5, help="attack steps")
parser.add_argument("--attack_type", type=str, default="fgsm")
parser.add_argument("--adversarial_training", type=bool, default=True)
# noise ratio
...
train_logger = Logger(task_name=task_name,
                      ...
                      heading=['epoch', 'train_loss', 'train_acc', 'val_loss', 'val_acc',
                               'test_loss', 'test_acc', "atest_loss", 'atest_acc'],
                      )
...
def train_model(model, _iter):
    ...
    for X, Y in tqdm(_iter):
        ...
        if params.adversarial_training and random.randint(0, 1) == 1:
...
def adversarial_samples(model, data, target, epsilon=params.strength):
...
def adversarial_test(model, test_loader):
    ...
    for data, target in tqdm(test_loader):
        ...
        perturbed_data = adversarial_samples(model, data, target)
        # init_output = model(data)
        # init_pred = init_output.max(1, keepdim=True)[1]
        perturbed_output = model(perturbed_data)
        perturbed_pred = perturbed_output.max(1, keepdim=True)[1].view(target.size())
        advloss = criterion(perturbed_output, target)
        correct += (target == perturbed_pred).float().sum().cpu().item()
        loss += advloss.item()

    # Calculate final accuracy for this epsilon
    final_acc = correct/count
    print("Epsilon: {:.4f}\tTest Accuracy = {} / {} = {}".format(params.strength, correct, count, final_acc))

    # Return the accuracy and an adversarial example
    return loss / len(test_loader), final_acc
...
if __name__ == "__main__":
    ...
    if not params.load:
        ...
        for epoch in range(params.num_epochs):
            ...
            atest_loss, atest_acc = adversarial_test(model, test_iter)
            train_logger.append(epoch + 1, train_loss, train_acc, val_loss, val_acc,
                                test_loss, test_acc, atest_loss, atest_acc)
